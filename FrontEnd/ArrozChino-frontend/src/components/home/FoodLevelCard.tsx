...
// Frontend web usa MQTT por WebSocket.
// La ESP32 usa broker.emqx.io:1883 por MQTT TCP normal.
const MQTT_URL =
...
function normalizeReading(payload: string): ReadingMessage | null {
  try {
    const parsed = JSON.parse(payload) as ReadingMessage;

    if (parsed.food_level === undefined && parsed.weight === undefined) {
      return null;
    }

    if (parsed.feeder_id && parsed.feeder_id !== FEEDER_ID) {
      return null;
    }
...
export default function FoodLevelCard() {
  ...
  function publishReadFoodCommand() {
    ...
    client.publish(COMMAND_TOPIC, "POSTSINGLE");

    console.log("Comando MQTT enviado: POSTSINGLE");
  ...
  async function handleRefreshClick() {
    ...
    const commandSent = publishReadFoodCommand();

    // Si MQTT no está conectado, se consulta directamente el backend.
    if (!commandSent) {
      await loadFeeder();
    }
  ...
  useEffect(() => {
    // Carga inicial.
    // Ya no se consulta cada 5 minutos: se actualiza cuando llega una lectura por MQTT.
    loadFeeder();
  }, [loadFeeder]);
  ...
  useEffect(() => {
    ...
    client.on("message", (_topic, payload) => {
      ...
      const reading = normalizeReading(payloadText);

      if (!reading) {
        console.log("Mensaje MQTT recibido sin lectura:", payloadText);
        return;
      }

      console.log("Lectura MQTT recibida:", reading);

      setFeeder((prev) => {
        if (!prev) return prev;

        const nextFoodLevel = Number(
          reading.food_level ?? reading.weight ?? prev.food_level
        );

        return {
          ...prev,
          food_level: Number.isFinite(nextFoodLevel)
            ? nextFoodLevel
            : prev.food_level,
        };
      });
      ...
      // Como el frontend no escucha la base de datos directamente,
      // cuando llega una lectura por MQTT sincroniza con backend.
      window.setTimeout(() => {
        loadFeeder();
      }, 500);
    });
    ...
    <section style={styles.section}>
      <div style={styles.card}>
        ...
        <div style={styles.dataRow}>
          <span>{formatKg(foodLevel)} disponibles</span>
          <span>{formatKg(foodLimit)} capacidad</span>
        </div>

        <p style={styles.helperText}>
          Se actualiza cuando llega una nueva lectura por MQTT y se sincroniza
        ...
        >
          ...
          >
            Enviando POSTSINGLE al comedero
          </span>
        </button>
      </div>
    </section>
